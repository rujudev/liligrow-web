...
if ( wc_get_page_id( 'cart' ) > 0 || wc_get_page_id( 'shop' ) > 0 ) : ?>
	<p class="return-to-shop">
		<a class="button wc-backward<?php echo esc_attr( wc_wp_theme_get_element_class_name( 'button' ) ? ' ' . wc_wp_theme_get_element_class_name( 'button' ) : '' ); ?>" href="<?php echo esc_url( apply_filters( 'woocommerce_return_to_shop_redirect', wc_get_page_permalink( 'shop' ) ) ); ?>">
			<?php
				 ...
				 */
				echo esc_html( apply_filters( 'woocommerce_return_to_shop_text', __( 'Go home', 'woocommerce' ) ) );
			?>
		</a>
	</p>
	<script>
		document.querySelector('#cart-content .woocommerce').setAttribute('style', 'display: block !important;');
	</script>
<?php endif; ?>
