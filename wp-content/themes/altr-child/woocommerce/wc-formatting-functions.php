 ...
 *
 * @param $option left|right|left_space|right_space
 * @return string
 */
...
function wc_format_price_range($from, $to)
{
    ...
    $price = sprintf(_x('%2$s &ndash; %1$s', 'Price range: from-to', 'woocommerce'), is_numeric($from) ? wc_price($from) : $from, is_numeric($to) ? wc_price($to) : $to);
...
function wc_esc_json($json, $html = false)
{
    return _wp_specialchars(
        ...
        // Escape quotes in attribute nodes only.
        'UTF-8',
        // json_encode() outputs UTF-8 (really just ASCII), not the blog's charset.
