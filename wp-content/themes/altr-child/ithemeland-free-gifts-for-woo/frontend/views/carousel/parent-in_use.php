...
foreach ($this->show_gift_item_for_cart as $gift_item_key => $gift) {
    ...
    } elseif (in_array($gift['key'], $count_info['gifts_set']) && $gift['can_several_gift'] == 'no') {
    ...
    if ($product_type == 'variable') {
        $gift_key = ($item_hover != 'disable-hover') ? $gift['item'] : '';
		$product_item .= apply_filters('it_free_gift_after_button_add_gift' ,'' , $gift['item'], $gift);		
        $product_item .= '
                                        <div class="wgb-add-gift-btn btn-select-gift-button" data-rule-id="' . esc_attr($gift['uid']) . '" data-id="' . esc_attr($gift_key) . '">
                                            <div class="wgb-loading-icon wgb-d-none">
                                                <div class="wgb-spinner wgb-spinner--2"></div>
                                            </div>
                                            <span>' . $select_gift . '</span>
                                        </div>';
    } else {
        $gift_key = ($item_hover != 'disable-hover') ? $gift['key'] : '';
		$product_item .= apply_filters('it_free_gift_after_button_add_gift' ,'' , $gift['item'], $gift);		
        $product_item .= '
                                        <div class="wgb-add-gift-btn btn-add-gift-button" data-id="' . esc_attr($gift_key) . '">
                                            <div class="wgb-loading-icon wgb-d-none">
                                                <div class="wgb-spinner wgb-spinner--2"></div>
                                            </div>
                                            <span>' . $add_gift . '</span>
