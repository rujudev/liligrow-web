...
<head>
	...
	<style>
        ...
        .woocommerce-Price-amount.amount bdi .integer,
        .woocommerce-Price-amount.amount bdi .decimal,
        .woocommerce-Price-amount.amount bdi .separator,
        ...
        .coupons button[name=pos_apply_coupon] {
            margin-top: 8px;
        }
		...
		table.shop_table thead .product-quantity,
		table.shop_table thead .product-total {
			width: 20%;	
		}
		...
		table.shop_table td.product-total #total-container {
		...
		table.shop_table td.product-total #total-container .includes_tax bdi *{
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        const cashOption = this.querySelector('#payment_method_pos_cash');
        const isCashOptionChecked = cashOption.getAttribute('checked');
        const montToPay = parentDocument.querySelector('div[dir=auto].css-146c3p1.r-gg3xrh.r-1x2decy.r-1i10wst.r-13uqrnb.r-b88u0q.r-rjixqe.r-q4m81j.r-3twk1y');
		const thTaxLabel = this.querySelectorAll('th[data-is_tax_column=true');
		montToPay.setAttribute('style', 'display: flex; flex-direction: column; gap: 1em;');

		if (thTaxLabel.length > 0) {
			thTaxLabel.forEach(taxLabel => {
				taxLabel.innerText = taxLabel.innerText.split(':').join(' incluido:');
			})
		}

        if (isCashOptionChecked) {
			const initialRefundText = 'Cambio: 0 €';
			const toRefundElement = this.querySelector('#to-refund');
            const cashInput = this.querySelector('#pos-cash-tendered');
            const total = <?php echo $order->total; ?>;
			const toRefund = document.createElement('span');
            toRefund.setAttribute('class', 'to-refund');			
			toRefund.innerHTML = initialRefundText;

			montToPay.appendChild(toRefund);	
            ...
            cashInput.addEventListener('input', function() {
                ...
                if (decimalIndex !== -1 && formattedValue.length - decimalIndex > 3) {
                ...
                if (parsedValue > total) {
                    const newTotal = String(parseFloat(parsedValue - total).toFixed(2)).replace('.', ',');
                    
					if (montToPay.getElementsByClassName('to-refund').length === 0) {
						montToPay.appendChild(toRefund);	
					}

					toRefund.innerText = `Cambio: ${newTotal} €`;
                } else {
					toRefund.innerText = initialRefundText;
				}
